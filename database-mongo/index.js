...
var ebay = require('./../server/ebayHelpers');
var fs = require('fs');
...
var saveRefinedAuction = auction => {
  var localImagePath = `./react-client/dist/local-gallery/${auction.itemId}.jpg`;
  var newAuction = new Auction(auction);
    ...
    .then(auction => {
      ...
      return new Promise((resolve, reject) => {
        ...
        readable.on('error', reject(() => {
          console.log('Error in saving auction: ', error);
          return auction;
        }));
      });
    });
...
var saveRawAuctions = ebayAuctions => {
  var refinedAuctions = ebay.refineData(ebayAuctions);
  var savedAuctions = [];
  refinedAuctions.forEach(auction => {
    savedAuctions.push(saveRefinedAuction(auction));
  });
  return Promise.all(savedAuctions);
}
